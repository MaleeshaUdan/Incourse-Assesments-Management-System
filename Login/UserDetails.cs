...
namespace Login
{
    public partial class UserDetails : Form
    {
        ...
        private void btnCreAdmin_Click(object sender, EventArgs e)
        {
            ...
            if(user!="" && pass!="" && passRe != "")
            {
                if (pass == passRe)
                {
                    ...
                    cmd.CommandText = "INSERT INTO Userlog VALUES('" + user + "','" + pass + "')";
                    cmd.ExecuteNonQuery();
                    con.Close();
                    display();
        ...
        private void display()
        {
            ...
            SqlCommand cmd = con.CreateCommand();
            cmd.CommandType = CommandType.Text;
            cmd.CommandText = "SELECT * FROM Userlog";
            cmd.ExecuteNonQuery();
        ...
        private void btnDelAdmin_Click(object sender, EventArgs e)
        {
            ...
            {
                SqlConnection con = new SqlConnection(@"Data Source=(LocalDB)\MSSQLLocalDB;AttachDbFilename=C:\ICA\Database\ICADB.mdf;Integrated Security=True;Connect Timeout=30");
                string query = "SELECT * FROM Userlog WHERE Username='" + user.Trim() + "'AND Password='" + pass.Trim() + "'";
                ...
                if (dtb.Rows.Count == 1)
                {
                    ...
                    SqlCommand cmd = con.CreateCommand();
                    cmd.CommandType = CommandType.Text;
                    cmd.CommandText = "DELETE FROM Userlog WHERE Username='" + user + "'"; ;
                    cmd.ExecuteNonQuery();
                    con.Close();
                    display();
        ...
        private void btnUpdPass_Click(object sender, EventArgs e)
        {
            ...
            if (user!="" && pass!="" && repass!="")
            {
                if (pass==repass)
                {
                    ...
                    SqlCommand cmd = con.CreateCommand();
                    cmd.CommandType = CommandType.Text;
                    cmd.CommandText = "UPDATE Userlog SET Password='" + pass + "'WHERE Username='" + user + "'";
                    cmd.ExecuteNonQuery();
                    con.Close();
                    display();
        ...
        private void btnCheckRec_Click(object sender, EventArgs e)
        {
            ...
            SqlCommand cmd = con.CreateCommand();
            cmd.CommandType = CommandType.Text;
            cmd.CommandText = "SELECT COUNT(*) AS COUNT FROM Userlog";
            cmd.ExecuteNonQuery();
            DataTable dt = new DataTable();
            SqlDataAdapter da = new SqlDataAdapter(cmd);
            da.Fill(dt);
